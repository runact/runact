...
/// Test that WebSocketServer works with runact's TcpStream via the adapter.
/// Client1 (std TcpStream) connects, sends a text frame, and client2
/// (also std TcpStream) receives the broadcasted message.
#[test]
fn test_runact_tcp_bridge() {
    let listener = runact::net::tcp_api::TcpListener::bind("127.0.0.1:0").expect("bind");
    ...
    let _server_thread = thread::spawn(move || {
        loop {
            ...
            thread::spawn(move || {
                ...
                let server = match WebSocketServer::bind(adapter, "/ws") {
                    Ok(s) => s,
                    Err(e) => {
    ...
    let client2_thread = thread::spawn(move || {
        ...
        // Read broadcast message from client1
        let mut buf = [0u8; 256];
    ...
    // --- Client 1 (sender) connects and sends a message ---
    {
        ...
        let frame = Frame {
            ...
            masked: false,
            mask_key: [0; 4],
            payload: b"hello via runact-bridge".to_vec(),
        };
    ...
    client2_thread.join().expect("client2 join");
