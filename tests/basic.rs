...
fn test_actor_inspection() {
    ...
    let actor_id = runtime
        ...
        .expect("Failed to spawn actor");
    let info = runtime
        .inspect_actor(actor_id)
        .expect("Failed to inspect actor");
...
fn test_send_fire_and_forget() {
    ...
    for _ in 0..5 {
        runtime
            .send(counter, CounterMessage::Increment)
            .expect("Failed to send");
    }

    // Verify by requesting the value
    let handle = runtime
        .request(counter, CounterMessage::GetValue)
...
fn test_request_blocking() {
    let mut runtime = Runtime::new().expect("Failed to create runtime");
    let counter = runtime
        .spawn(CounterActor { count: 99 })
        .expect("Failed to spawn");

    // Blocking request with timeout
    let count: u64 = runtime
        .request_blocking(
            counter,
            CounterMessage::GetValue,
...
fn test_actor_to_actor_communication() {
    ...
    let forwarder = runtime
        .spawn(ForwardActor { target: None })
        .expect("Failed to spawn forwarder");

    // Set the forwarder's target to the logger
    runtime
        .send(forwarder, ForwardMessage::SetTarget(logger))
        .expect("Failed to send");

    // Send through the forwarder
    runtime
        .send(forwarder, ForwardMessage::Forward("hello".to_string()))
        .expect("Failed to send");
    runtime
        .send(forwarder, ForwardMessage::Forward("world".to_string()))
        .expect("Failed to send");
    ...
    let handle = runtime
        .request(logger, LoggerMessage::GetCount)
        .expect("Failed to request");
    let reply = handle
        .recv_timeout(Duration::from_secs(1))
        .expect("Failed to receive reply");
